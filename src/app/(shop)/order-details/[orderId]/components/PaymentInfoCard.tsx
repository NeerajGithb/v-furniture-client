...
import { getReadablePaymentMethod, getReadablePaymentStatus } from '../utils/orderStatusConfig';
...
export function PaymentInfoCard({ order }: PaymentInfoCardProps) {
    return (
        <>
            ...
            >
                <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <span
                            className={`px-3 py-1 text-sm font-semibold rounded-full ${order.paymentStatus === 'paid'
                    ...
                    {order.paymentMethod === 'cod' && (
                        <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xs p-3 mt-3">
                            <div className="flex items-start gap-2">
                                <Banknote size={16} className="text-gray-700 dark:text-gray-300 mt-0.5 shrink-0" />
                                <div>
                                    <p className="text-gray-900 dark:text-white font-medium text-sm">Cash on Delivery</p>
            ...
            {/* Shipping Address */}
            <Card title="Delivery Address" icon={<MapPin size={18} className="text-gray-900 dark:text-white" />}>
                <div className="space-y-2">
                    <p className="font-semibold text-gray-900 dark:text-white">{order.shippingAddress.fullName}</p>
                    <p className="text-gray-700 dark:text-gray-300">{order.shippingAddress.addressLine1}</p>
                    {order.shippingAddress.addressLine2 && (
                        <p className="text-gray-700 dark:text-gray-300">{order.shippingAddress.addressLine2}</p>
                    )}
                    <p className="text-gray-700 dark:text-gray-300">
                        {order.shippingAddress.city}, {order.shippingAddress.state}
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">PIN: {order.shippingAddress.postalCode}</p>
