...
export function OrderProgress({ statusSteps, isOrderActive, isOverdue = false }: OrderProgressProps) {
                    ...
                    {statusSteps.map((step, index) => (
                        <div key={step.key} className="flex items-start gap-3 relative pb-6 last:pb-0">
                            ...
                            {index < statusSteps.length - 1 && (
                                <div
                                    className={`absolute left-4 top-8 w-0.5 h-full ${
                                        step.completed ? 'bg-gray-900 dark:bg-gray-300' : 'bg-gray-200 dark:bg-gray-700'
                                    }`}
                                />
                            )}
                            ...
                            <div className="flex-1 pt-0.5">
                                <div className="flex items-center justify-between mb-1">
                                    <h4
                                        className={`font-semibold text-sm ${
                                            step.completed || step.active ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'
                                        }`}
                                    >
                                        {step.label}
                                    </h4>
                                ...
                                <p
                                    className={`text-xs ${
                                        step.completed || step.active ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500'
                                    }`}
                                >
                                    {step.description}
                                </p>
