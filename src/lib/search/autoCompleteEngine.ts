...
export class AutoCompleteEngine {
  private static memoryCache: MemoryCache | null = null;
  private static readonly CACHE_DURATION = 30 * 60 * 1000; // 30 minutes
  private static readonly MIN_RESULTS = 6;
  ...
  private static readonly SIZE_KEYWORDS = [
    "king",
    "queen",
    "single",
    "double",
   ...
   */
  static async getAutocomplete(query: string): Promise<AutocompleteItem[]> {
    try {
      if (!query || query.length < 2) {
        return [];
      }

      await this.ensureMemoryCacheLoaded();

      const context = this.analyzeQuery(query);
      const autocomplete = this.buildContextAwareSuggestions(query, context);
  ...
  /**
   * Ensure memory cache is loaded and fresh
   */
  private static async ensureMemoryCacheLoaded(): Promise<void> {
    const now = Date.now();

    if (
      this.memoryCache &&
      now - this.memoryCache.lastUpdate < this.CACHE_DURATION
    ) {
      return;
    }
    ...
    const [categories, subcategories, inspirations, products] =
      await Promise.all([
        ...
        Product.find({ isActive: true, isPublished: true, status: "APPROVED" })
          .select(
            "name slug brand material colorOptions size tags totalSold isFeatured isBestSeller isNewArrival categorySlug subcategorySlug searchKeywords mainImage galleryImages attributes finalPrice originalPrice",
          )
    ...
    products.forEach((p: any) => {
      ...
      if (p.attributes?.material)
        materialMap.set(
          ...
          (materialMap.get(p.attributes.material) || 0) + 1,
        );

      if (p.colorOptions?.length) {
        p.colorOptions.forEach((c: string) =>
      ...
      if (p.attributes?.color) {
        colorMap.set(
          p.attributes.color,
          (colorMap.get(p.attributes.color) || 0) + 1,
        );
      }
    });
  ...
  /**
   * Analyze query to extract context and constraints
   */
  private static analyzeQuery(query: string): QueryContext {
    ...
    this.PRICE_KEYWORDS.forEach((keyword) => {
      const keywordPattern = keyword.replace(/\s+/g, "\\s+");
      const regex = new RegExp(
        `\\b${keywordPattern}\\s+(rs\\.?\\s*)?([\d,]+k?)\\b`,
        "i",
      );
      ...
      if (match) {
        ...
        if (value) {
          context.priceConstraint = {
            value,
            operator: ["under", "below", "less", "upto", "up"].includes(
              keyword.split(" ")[0],
            )
              ? "under"
              : ["above", "over", "more"].includes(keyword.split(" ")[0])
                ? "above"
                : "around",
          };
        }
    ...
    const betweenMatch = queryLower.match(
      /between\s+(rs\.?\s*)?([\d,]+k?)\s+(?:and|to|-)\s+(rs\.?\s*)?([\d,]+k?)/i,
    );
    if (betweenMatch) {
      const min = parseInt(
        betweenMatch[2].replace(/,/g, "").replace("k", "000"),
      );
      const max = parseInt(
        betweenMatch[4].replace(/,/g, "").replace("k", "000"),
      );
      context.priceConstraint = {
        value: min,
        operator: "under",
    ...
    // Detect materials
    this.MATERIAL_KEYWORDS.forEach((material) => {
    ...
    // Detect sizes
    this.SIZE_KEYWORDS.forEach((size) => {
    ...
    // Detect styles
    this.STYLE_KEYWORDS.forEach((style) => {
    ...
    // Detect rooms
    this.ROOM_KEYWORDS.forEach((room) => {
    ...
    // Detect brands
    if (this.memoryCache) {
      this.memoryCache.brands.forEach((b) => {
    ...
    if (mainWords.length > 0) {
      ...
      if (this.memoryCache) {
        ...
        if (mainWords.length >= 2) {
          ...
          const matchesCategory = this.memoryCache.categories.some(
            (c) =>
              c.name.toLowerCase().includes(twoWordCombo) ||
              twoWordCombo.includes(c.name.toLowerCase()),
          );
          const matchesSubcategory = this.memoryCache.subcategories.some(
            (sc) =>
              sc.name.toLowerCase().includes(twoWordCombo) ||
              twoWordCombo.includes(sc.name.toLowerCase()),
          );
        ...
        for (const word of mainWords) {
          ...
          const matchesSubcategory = this.memoryCache.subcategories.some(
            (sc) =>
              sc.name.toLowerCase().includes(word) ||
              word.includes(sc.name.toLowerCase()),
          );
  ...
  /**
   * Build context-aware autocomplete suggestions
   */
  private static buildContextAwareSuggestions(
    query: string,
    ...
    if (hasTrailingSpace && queryLower.trim().length > 0) {
      ...
      const matchingCategory = this.memoryCache.categories.find(
        (c) => c.name.toLowerCase() === queryLower.trim(),
      );
      if (matchingCategory) {
      ...
      if (!exactImage) {
        const matchingSubcategory = this.memoryCache.subcategories.find(
          (sc) => sc.name.toLowerCase() === queryLower.trim(),
        );
      ...
      if (!exactImage) {
        const matchingProduct = this.memoryCache.products.find(
          (p) => p.name.toLowerCase() === queryLower.trim(),
        );
        if (matchingProduct) {
    ...
    if (context.mainEntity) {
      const relevantProducts = this.memoryCache.products.filter((p) => {
        ...
        const inKeywords = p.searchKeywords?.some((k) =>
        ...
        if (context.materialConstraint) {
          const materialMatch = Boolean(
            ...
            p.attributes?.material
              ?.toLowerCase()
              .includes(context.materialConstraint),
          );
        ...
        if (context.sizeConstraint) {
          const sizeMatch = Boolean(
            p.size?.some((s) =>
              s.toLowerCase().includes(context.sizeConstraint!),
            ) ||
        ...
        if (context.styleConstraint) {
          const styleMatch = Boolean(
            p.tags?.some((t) =>
              t.toLowerCase().includes(context.styleConstraint!),
            ) ||
            p.attributes?.style
              ?.toLowerCase()
              .includes(context.styleConstraint!) ||
        ...
        if (context.roomConstraint) {
          const roomMatch = Boolean(
            p.tags?.some((t) =>
              t.toLowerCase().includes(context.roomConstraint!),
            ) ||
            p.attributes?.room
              ?.toLowerCase()
              .includes(context.roomConstraint!) ||
        ...
        if (context.priceConstraint) {
          ...
          if (context.priceConstraint.operator === "under") {
            ...
            if (context.priceConstraint.max) {
              priceMatch =
                price >= context.priceConstraint.value &&
                price <= context.priceConstraint.max;
            }
            ...
            priceMatch =
              Math.abs(price - context.priceConstraint.value) <= range;
          }
    ...
    this.memoryCache.categories.forEach((c) => {
    ...
    this.memoryCache.subcategories.forEach((sc) => {
    ...
    this.memoryCache.materials.forEach((m) => {
    ...
    this.memoryCache.colors.forEach((c) => {
    ...
    if (
      suggestions.length < this.MIN_RESULTS &&
      (hasPrice ||
        hasColor ||
        hasMaterial ||
        hasSize ||
      ...
      this.buildConstraintSuggestions(context, addUnique);
    }

    // Priority 5: Inspirations (must include query)
    this.memoryCache.inspirations.forEach((i) => {
    ...
    if (suggestions.length < this.MIN_RESULTS && context.mainEntity) {
      this.addEntityVariations(context.mainEntity, addUnique, context);
    }

    // Return only MIN_RESULTS items (filtering already done in addUnique)
    return suggestions.slice(0, this.MIN_RESULTS);
  }

  /**
   * Build constraint-based suggestions
   */
  private static buildConstraintSuggestions(
    context: QueryContext,
    addUnique: (
      text: string,
      type: AutocompleteItem["type"],
    ...
    if (priceConstraint) {
      const ranges = this.generatePriceRanges(priceConstraint.value);
      ...
      if (colorConstraint) {
        addUnique(
          `${colorConstraint} ${mainEntity} under ${priceConstraint.value}`,
          "product",
        );
      }
      if (materialConstraint) {
        addUnique(
          `${materialConstraint} ${mainEntity} under ${priceConstraint.value}`,
          "product",
        );
      }
      if (sizeConstraint) {
        addUnique(
          `${sizeConstraint} ${mainEntity} under ${priceConstraint.value}`,
          "product",
        );
      }
      if (brandConstraint) {
        addUnique(
          `${brandConstraint} ${mainEntity} under ${priceConstraint.value}`,
          "product",
        );
      }
      if (styleConstraint) {
        addUnique(
          `${styleConstraint} ${mainEntity} under ${priceConstraint.value}`,
          "product",
        );
      }
    }
  ...
  /**
   * Generate price ranges based on base price
   */
  private static generatePriceRanges(basePrice: number): number[] {
    const ranges: number[] = [];
   ...
   */
  private static addEntityVariations(
    entity: string,
    addUnique: (
      text: string,
      type: AutocompleteItem["type"],
    ...
    const matchingProducts = this.memoryCache.products.filter((p) =>
    ...
    matchingProducts.forEach((p) => {
      ...
      const matchingWord = words.find(
        (w) => w.includes(entityLower) || entityLower.includes(w),
      );
      if (matchingWord) {
        fullEntityWords.set(
          matchingWord,
          (fullEntityWords.get(matchingWord) || 0) + 1,
        );
      }
    });
    ...
    if (fullEntityWords.size > 0) {
      fullEntityWord = Array.from(fullEntityWords.entries()).sort(
        (a, b) => b[1] - a[1],
      )[0][0];
    }
    ...
    matchingProducts.forEach((p) => {
      ...
      const entityIndex = words.findIndex(
        (w) => w.includes(entityLower) || entityLower.includes(w),
      );
    ...
    if (productTypes.size > 0) {
      const mostCommonType = Array.from(productTypes.entries()).sort(
        (a, b) => b[1] - a[1],
      )[0][0];
    ...
    matchingProducts.forEach((p) => {
      ...
      if (p.attributes?.material)
        materialsSet.add(p.attributes.material.toLowerCase());
    });
    ...
    const getUniqueProduct = (
      filter?: (p: CachedProduct) => boolean,
    ...
    topColors.forEach((color) => {
      ...
      const matchingProduct = getUniqueProduct(
        (p) =>
          p.colorOptions?.some((c) => c.toLowerCase() === color) ||
          p.attributes?.color?.toLowerCase() === color,
      );
      const productIndex = matchingProduct
        ? matchingProducts.indexOf(matchingProduct)
        : -1;
    ...
    if (prices.length > 0) {
      ...
      priceRanges.forEach((priceLimit) => {
        ...
        const matchingProduct = getUniqueProduct(
          (p) => (p.finalPrice || p.originalPrice || 0) <= priceLimit,
        );
        const productIndex = matchingProduct
          ? matchingProducts.indexOf(matchingProduct)
          : -1;
        const image =
          productIndex >= 0 ? getNextImage(productIndex) : undefined;
        addUnique(
          suggestionText,
          "product",
          undefined,
