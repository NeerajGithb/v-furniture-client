...
export const getUserFriendlyErrorMessage = (error: string): string => {
  ...
  if (lowerError.includes('insufficient stock') || lowerError.includes('out of stock')) {
    return 'Some items in your order are currently out of stock. Please update your cart.';
  }

  // Address errors
  if (lowerError.includes('address not found') || lowerError.includes('invalid address')) {
  ...
  // Validation errors
  if (lowerError.includes('validation failed') || lowerError.includes('invalid data')) {
    return 'Please check your order details and try again.';
  }

  // Order creation errors (generic)
  if (lowerError.includes('order creation failed') || lowerError.includes('failed to create order')) {
