...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
    ...
    const handleEmailAuth = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (authState.isLogin) {
                const success = await handleLogin(email, password, setError, setIsOAuth);
                if (success) {
                    setTimeout(() => handleClose(), 100);
                }
            } else {
                const result = await handleSignup(name, email, password, confirmPassword, setError);
                ...
                if (result && typeof result === 'object' && 'requiresVerification' in result && result.requiresVerification) {
                    ...
                    // OAuth user or verification not required - logged in immediately
                    setTimeout(() => handleClose(), 100);
                }
            }
        } finally {
    ...
    const handleGoogleLogin = () => {
        ...
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'visible' && window.location.href === originalUrl) {
                setTimeout(() => {
                    setLoading(false);
                    setIsOAuth(false);
                }, 500);
    ...
    const handleVerifyCodeSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setResetCodeError(err.message || 'Verification failed');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleVerifyEmailSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await handleVerifyEmailCode(verificationState.email, verificationState.code);
            ...
            setVerificationCodeError(err.message || 'Verification failed');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleResendVerificationCode = async () => {
        try {
            await handleSendVerificationCode(verificationState.email, verificationState.name);
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    className={`bg-white dark:bg-gray-900 rounded-sm shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex relative ${authState.loading ? 'pointer-events-none opacity-90' : ''
                        }`}
