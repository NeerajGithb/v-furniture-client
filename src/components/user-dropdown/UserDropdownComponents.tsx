...
export const UserProfileSection = ({ user }: UserProfileSectionProps) => (
  <motion.div
    ...
    className="px-3 py-2 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900"
  >
...
export const DropdownMenuItem = ({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={isLoading}
      className={`w-full flex items-center gap-2 px-3 py-1.5 text-left text-xs transition-none ${
        isLoading
          ? "cursor-not-allowed opacity-60"
          : isActive
            ? "bg-gray-100 dark:bg-gray-700 text-black dark:text-white font-semibold cursor-default"
    ...
    >
      ...
      >
        {isLoading ? (
          ...
          <IconComponent
            className={`w-3.5 h-3.5 ${isActive ? "text-black dark:text-white" : "text-gray-600 dark:text-gray-400"}`}
          />
        )}
      </motion.div>
      ...
      {item.badge && !isLoading && (
        <motion.span
          ...
          className="text-xs font-medium text-black dark:text-white leading-none"
        >
          {item.badge}
        </motion.span>
      )}
...
export const LogoutButton = ({ isLoggingOut, onLogout }: LogoutButtonProps) => (
  <motion.div
    variants={itemVariants}
    className="border-t border-gray-100 dark:border-gray-700 py-0.5"
  >
    <motion.button
      onClick={onLogout}
      disabled={isLoggingOut}
      className={`w-full flex items-center gap-2 px-3 py-1.5 text-left text-xs font-medium transition-none ${
        isLoggingOut
